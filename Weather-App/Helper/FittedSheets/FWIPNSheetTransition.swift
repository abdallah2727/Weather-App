//
//  FWIPNSheetTransitioningDelegate.swift
...
public class FWIPNSheetTransition: NSObject, UIViewControllerAnimatedTransitioning {
    var presenting = true
    weak var presenter: UIViewController?
    var options: FWIPNSheetOptions
    ...
    init(options: FWIPNSheetOptions) {
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if self.presenting {
            guard let presenter = transitionContext.viewController(forKey: .from), let sheet = transitionContext.viewController(forKey: .to) as? FWIPNSheetViewController else {
                transitionContext.completeTransition(true)
                return
            }
            self.presenter = presenter
            
            if FWIPNSheetOptions.shrinkingNestedPresentingViewControllers {
                FWIPNSheetTransition.currentPresenters.append(presenter)
            }
            ...
            FWIPNConstraints(for: sheet.view) {
            ...
            let sheet = transitionContext.viewController(forKey: .from) as? FWIPNSheetViewController else {
                transitionContext.completeTransition(true)
                return
            }
    ...
    func restorePresentor(_ presenter: UIViewController, animated: Bool = true, animations: (() -> Void)? = nil, completion: ((Bool) -> Void)? = nil) {
        FWIPNSheetTransition.currentPresenters.removeAll(where: { $0 == presenter })
        let topSafeArea = UIApplication.shared.windows.filter {$0.isKeyWindow}.first?.FWIPNcompatibleSafeAreaInsets.top ?? 0
        UIView.animate(
            withDuration: self.options.transitionDuration,
            animations: {
                if self.options.shrinkPresentingViewController {
                ...
                if FWIPNSheetOptions.shrinkingNestedPresentingViewControllers {
                    ...
                    let presenters = FWIPNSheetTransition.currentPresenters.reversed()
    ...
    func setPresentor(percentComplete: CGFloat) {
        ...
        if FWIPNSheetOptions.shrinkingNestedPresentingViewControllers {
            let presenters = FWIPNSheetTransition.currentPresenters.reversed().dropFirst()
            for lowerPresenter in presenters {
                scale *= 0.92
