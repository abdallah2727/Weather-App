//
//  FWIPNSheetContentViewController.swift
...
public class FWIPNSheetContentViewController: UIViewController {
    
    public private(set) var childViewController: UIViewController
    
    private var options: FWIPNSheetOptions
    ...
    public var gripSize: CGSize = CGSize(width: 50, height: 6) {
        didSet {
            self.gripSizeFWIPNConstraints.forEach({ $0.isActive = false })
            FWIPNConstraints(for: self.gripView) {
                self.gripSizeFWIPNConstraints = $0.size.set(self.gripSize)
            }
    ...
    public var treatPullBarAsClear: Bool = FWIPNSheetViewController.treatPullBarAsClear {
        didSet {
    ...
    weak var delegate: FWIPNSheetContentViewDelegate?
    ...
    private var gripSizeFWIPNConstraints: [NSLayoutConstraint] = []
    ...
    public init(childViewController: UIViewController, options: FWIPNSheetOptions) {
    ...
    private func setupOverflowView() {
        switch (self.options.FWIPNTransitionOverflowType) {
    ...
    private func setupChildViewController() {
        ...
        FWIPNConstraints(for: self.childViewController.view) { view in
        ...
        if self.options.shouldExtendBackground, self.options.pullBarHeight > 0 {
            self.childViewController.FWIPNcompatibleAdditionalSafeAreaInsets = UIEdgeInsets(top: self.options.pullBarHeight, left: 0, bottom: 0, right: 0)
        }
        
        self.childViewController.didMove(toParent: self)
        
        self.childContainerView.layer.masksToBounds = true
        self.childContainerView.layer.FWIPNcompatibleMaskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner]
    }

    private func setupContentView() {
        self.view.addSubview(self.contentView)
        FWIPNConstraints(for: self.contentView) {
        ...
        self.contentWrapperView.layer.FWIPNcompatibleMaskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner]
    ...
    private func setupChildContainerView() {
        ...
        FWIPNConstraints(for: self.childContainerView) { view in
    ...
    private func setupPullBarView() {
        ...
        FWIPNConstraints(for: pullBarView) {
        ...
        // This will be overriden whenever the sizes property is changed on FWIPNSheetViewController
//        pullBarView.accessibilityLabel = FWIPNLocalize.dismissPresentation.FWIPNLocalized
...
extension FWIPNSheetContentViewController: UINavigationControllerDelegate {
